...
export function SleepComponent() {
  return (
    <div className="sleep-card-tilt w-[290px] rounded-[1.75rem] bg-white p-5 shadow-[0_30px_60px_-15px_rgba(15,40,50,0.25)] ring-1 ring-black/5 sm:w-[330px]">
      ...
      {/* phases legend */}
      <div className="mt-3 flex items-center justify-center gap-x-5 gap-y-2 text-[10px] font-bold text-neutral-500">
        {phases.map((p) => (
          <div key={p.key} className="flex items-center gap-1.5">
            <span className="size-2 rounded-full" style={{ backgroundColor: p.color }} />
            <span>
              {p.label} <span className="font-extrabold text-neutral-800">{p.time}</span>
            </span>
          </div>
        ))}
      </div>
